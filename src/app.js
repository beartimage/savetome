import './styles.css';
    ...
    function recordTagOverride(domain, tag, action) {
      if (!domain) return;
    ...
    function itemTimestamp(item) { return item.added || (item.id > 1e12 ? item.id : null); }
    function timeAgo(ts) {
      if (!ts) return '';
    ...
    /* ==========================================================================
       SMART TAGS — on-device link classifier
       No backend, no API calls: a curated knowledge base + weighted token
       matching over the URL (and any known title/note). Produces normalized,
       de-duplicated, ranked tags + a human description entirely in the browser.
       ========================================================================== */
    ...
    function hostMatchesPattern(host, m) {
    ...
    function prettifyTitle(path, domain) {
    ...
    function generateLinkMetadata(url, domain, path, title) {
